...
class MultiChainDataAggregator:
    ...
    # async def _collect_data_with_retry(self, source_name: str, address: str, collection_func) -> Dict[str, Any]:
        # """Enhanced data collection with retry logic and performance monitoring"""
        
        # start_time = datetime.now()
        
        # for attempt in range(self.max_retries):
        #     try:
        #         logger.info(f"🔄 {source_name} attempt {attempt + 1}/{self.max_retries}")
                
        #         # Execute collection with individual timeout
        #         result = await asyncio.wait_for(
        #             collection_func(address),
        #             timeout=60  # 60 second timeout per source
        #         )
        ...
        #                 'success': False,
        #                 'data': {},
        #                 'error': f'{source_name} timeout after {self.max_retries} attempts',
        #                 'attempts': self.max_retries,
        #                 'collection_time': collection_time
        #             }
...
# async def _collect_data_with_retry(self, source_name: str, address: str, collection_func) -> Dict[str, Any]:
#     """Enhanced data collection with optimized timeouts"""
...
#             # Execute collection with REDUCED individual timeout
#             result = await asyncio.wait_for(
#                 collection_func(address),
#                 timeout=self.individual_client_timeout  # 20 second timeout per client
#             )
...
#             await asyncio.sleep(self.retry_delay)  # Reduced delay
