...
# class AlchemyClient:
#     """
#     Enhanced Alchemy API client for comprehensive multi-chain transaction data
...
#                 'native_token': 'ETH',
#                 'usd_price': 3500  # Approximate - in production use real-time price feeds
#             },
#             'polygon': {
...
#         """Enhanced chain data fetching with retry logic"""
...
#         """Get chain data with retry logic"""
        
#         for attempt in range(self.max_retries):
#             try:
#                 # Get transaction count
#                 tx_count = await self._get_transaction_count(session, address, config['rpc_url'])
                
#                 # Get asset transfers (enhanced with more data)
#                 transfers = await self._get_enhanced_asset_transfers(session, address, config['rpc_url'])
                
#                 # Get gas data and pricing
#                 gas_data = await self._get_enhanced_gas_data(session, address, config['rpc_url'], config)
                
#                 # Get token balances
#                 token_balances = await self._get_token_balances(session, address, config['rpc_url'])
...
#     async def _get_transaction_count(self, session, address: str, rpc_url: str) -> int:
#         """Fetch total transaction count for address"""
#         # Your existing implementation or correct one
...
#     def _process_chain_results(self, chain_results: list) -> dict:
#         # Process and collate the results from each chain for metrics
#         # Include your previously defined processing logic here
#         return {
#             "successful_chains": [],
#             "failed_chains": [],
...
#     #     """Enhanced asset transfers with better categorization"""
...
#     #             "maxCount": "0xC8",  # 200 transactions for better analysis
#     #             "order": "desc"
#     #         }),
#     #         self._fetch_transfers(session, rpc_url, {
#     #             "fromBlock": "0x0",
...
#     #             "toAddress": address,
#     #             "category": ["external", "internal", "erc20", "erc721", "erc1155"],
#     #             "maxCount": "0xC8",  # 200 transactions
#     #             "order": "desc"
#     #         })
#     #     ]
        
#     #     from_transfers, to_transfers = await asyncio.gather(*transfer_tasks)
...
#     # In clients/alchemy_client.py, update the _get_enhanced_asset_transfers method:

# async def _get_enhanced_asset_transfers(self, session: aiohttp.ClientSession, 
#                                       address: str, rpc_url: str, chain_name: str) -> List[Dict]:
#     """Enhanced asset transfers with chain-specific category filtering"""
    
#     # Define supported categories per chain
#     chain_categories = {
...
#             "toBlock": "latest",
#             "fromAddress": address,
#             "category": supported_categories,  # Use chain-specific categories
#             "maxCount": "0xC8",
#             "order": "desc"
#         }),
#         self._fetch_transfers(session, rpc_url, {
...
#     #     """Enhance transfer data with additional analytics"""
...
#     #             enhanced['date'] = datetime.fromtimestamp(timestamp).isoformat()
#     #         except:
...
#             # For other tokens, would need price oracle
#             return 0.0
#         except:
#             return 0.0
...
#         """Enhanced gas data with efficiency metrics"""
...
#                 # Efficiency score (lower congestion = higher efficiency)
...
#         """Calculate enhanced transaction metrics with advanced analytics"""
        
#         all_transfers = processed_data['all_transfers']
        
#         # Basic metrics
#         total_transactions = processed_data['total_transactions']
#         active_chains = len(processed_data['successful_chains'])
...
#         cross_chain_metrics = self._analyze_cross_chain_activity(processed_data['chain_distribution'])
        
#         # Gas efficiency across chains
#         gas_metrics = self._analyze_gas_efficiency(processed_data['gas_data'])
...
#             'chain_distribution': processed_data['chain_distribution'],
#             'transaction_types': pattern_metrics['transaction_types'],
#             'recent_activity': activity_trends,
#             'successful_chains': processed_data['successful_chains'],
#             'failed_chains': processed_data['failed_chains'],
...
#         timestamps = [t.get('timestamp', 0) for t in transfers if t.get('timestamp')]
...
#         successful_chains = len(processed_data['successful_chains'])
#         quality_score += min(30, successful_chains * 7.5)
        
#         # Transaction data availability (max 25 points)
#         if processed_data['total_transactions'] > 0:
#             quality_score += 25
        
#         # Transfer data quality (max 25 points)
#         transfer_count = len(processed_data['all_transfers'])
#         quality_score += min(25, transfer_count * 0.5)
        
#         # Gas data availability (max 20 points)
#         gas_data_chains = len([c for c in processed_data['gas_data'].values() if c.get('current_gas_price_wei', 0) > 0])
...
#             else:
#                 processed_data['failed_chains'].append({
#                     'chain': result.get('chain_name', 'unknown'),
...
#         """Fetch transfers using alchemy_getAssetTransfers"""
...
#             data = await response.json()
#             if 'error' in data:
#                 logger.warning(f"Asset transfer fetch error: {data['error']}")
#                 return []
...
#     """Comprehensive test for enhanced Alchemy client"""
    
#     client = AlchemyClient()
#     test_address = "0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045"  # Vitalik's address
    
#     try:
#         print("🧪 Testing Enhanced Alchemy Client...")
#         print("=" * 50)
